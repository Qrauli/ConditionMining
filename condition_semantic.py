...
from ruleforge.structures import RuleDetail, ConditionalRuleDetail
from ruleforge.utils.utils import create_model_for_process, invoke_with_retries


from ruleforge.utils.log import setup_logging

cs_logger = setup_logging(log_directory='logs/condition_suggestion',
                          logger_name='condition_suggestion', level=logging.DEBUG)
...
async def analyze_semantic_relevance(
    ...
    **Examples of Correct Analysis**
    ...
    try:
        result = invoke_with_retries(structured_llm, prompt_content, logger=cs_logger, timeout_seconds=60)
        return result
    except Exception as e:
        cs_logger.exception("Error during semantic relevance analysis.")
        return None
...
async def analyze_column_utility(
    ...
    **Output:**
    ...
    try:
        result = invoke_with_retries(structured_llm, prompt_content, logger=cs_logger, timeout_seconds=45)
        return result
    except Exception as e:
        cs_logger.exception("Error during column utility analysis.")
        return None
...
async def verify_condition_semantically(
    ...
    **Verdict:**
    ...
    try:
        result = invoke_with_retries(structured_llm, prompt, logger=cs_logger, timeout_seconds=30)
        return result
    except Exception as e:
        cs_logger.error(f"Error validating condition: {e}")
