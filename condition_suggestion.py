...
cs_logger = setup_logging(log_directory='logs/rule refiner',
                          logger_name='rule refiner', level=logging.DEBUG)
...
def suggest_conditions_for_single_row_rule(
    ...
    if not violating_samples or not satisfying_samples:
        cs_logger.warning("Cannot suggest conditions with empty violating or satisfying samples.")
        return []
    ...
    cs_logger.info("Generating and evaluating atomic masks...")
    ...
    cs_logger.info("Generating and evaluating merged conditions...")
    ...
    cs_logger.info(f"Evaluating {len(merged_candidates_to_eval)} merged conditions after limiting...")
    ...
    if combinations_to_test:
        cs_logger.info(f"Evaluating {len(combinations_to_test)} unique conjunctive conditions (Hybrid Search)...")
        conjunctive_results = []
    ...
    cs_logger.info(f'$$$$$$$$ Final suggested single-row conditions: ({len(filtered)} found) $$$$$$$$$')
    return filtered
...
def _create_pairs_dataframe(df: pd.DataFrame, strategy: str, max_pairs_limit: int = 5_000_000) -> pd.DataFrame:
    ...
    if estimated_pairs > max_pairs_limit:
        cs_logger.warning(f"Pair generation estimate {estimated_pairs} > limit {max_pairs_limit}. Downsampling.")
    ...
    try:
        ...
        cs_logger.error("MemoryError during pair generation.")
        return pd.DataFrame()
...
def suggest_conditions_for_multi_row_rule(
    ...
    cs_logger.info("Preparing data for vectorized evaluation...")
    ...
    cs_logger.info(f"Evaluating {len(conditions_to_eval)} atomic conditions...")
...
# --- Step 3: Merged Conditions (Reuses Cache) ---
    ...
    if merged_candidates:
        cs_logger.info(f"Evaluating {len(merged_candidates)} merged conditions...")
    ...
    if conjunctive_candidates:
        cs_logger.info(f"Evaluating {len(conjunctive_candidates)} unique conjunctive conditions (Hybrid Search)...")
    ...
    cs_logger.info(f'$$$$ Final suggested multi-row conditions: ({len(filtered)} found) $$$$')
    for cond in filtered[:10]:  # Log only top 10 for brevity
        cs_logger.debug(f"Condition: {cond['condition']}, Score: {cond['score']}, "
                       f"Conf: {cond['confidence']}, Pen: {cond['penalty']}")
    return filtered
