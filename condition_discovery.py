...
import pandas as pd
from ruleforge.rule_refinement.condition_suggestion import ConjunctiveCondition, _flatten_multi_row_samples_to_df, suggest_conditions_for_multi_row_rule, suggest_conditions_for_single_row_rule
from ruleforge.structures import RuleDetail
from ruleforge.rule_refinement.condition_semantic import analyze_column_utility, analyze_semantic_relevance, verify_condition_semantically
...
rr_logger = setup_logging(log_directory='logs/rule refiner',
...
class ConditionDiscoveryEngine:
    ...
    async def discover_conditions_for_rule(self, rule_detail: RuleDetail, 
        ...
        if self.use_llm:
            ...
            rr_logger.info("LLM disabled (ablation mode): using ALL columns without semantic filtering.")
            relevant_cols_set = set(categorical_columns.keys()) | set(numerical_columns)
